...
path = '..\\..\\Files_generated\\Classifiers_results\\Game_by_game_threshold_voting\\Neural_network_2\\'
...
for idx, g in enumerate(games):
    ...
    for i in range(1, predictions_per_game+1):
        single_game_result = fit_model(X_train_val, X_test_val, y_train_val, y_test_val_categorical,
                                       number_of_features_val, threshold_val)
        single_game_votes = process_single_game_result(i, single_game_votes, single_game_result)

    single_game_overall_res = process_final_result_for_single_game(int(y_test_val_binary[0]), single_game_votes)
